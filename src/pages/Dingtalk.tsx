...
export default function Dingtalk({ showToast }: { showToast: ShowToast }) {
  ...
  const configureChannel = async () => {
    ...
    try {
      ...
      await invoke('configure_dingtalk_channel', {
        clientId, clientSecret, corpId, agentId, dmPolicy, groupPolicy,
      });
      setCredsSaved(true);
    ...
    <>
      ...
      <div className="card" style={{ borderColor: 'rgba(74,158,255,0.3)', background: 'rgba(74,158,255,0.05)', marginBottom: 16 }}>
        <div style={{ fontSize: 13, color: 'var(--muted2)', lineHeight: 1.9 }}>
          ...
          配置需要先安装社区插件，步骤 2–3 需在钉钉开放平台手动完成，标有 <ManualTag>手动</ManualTag> 标记。
        </div>
      </div>
      ...
      <div className="steps">
        ...
        {/* Step 1: 安装插件 */}
        <StepCard
          num={1} title="安装钉钉 Channel 插件"
          desc="钉钉支持通过社区插件接入，需先安装。安装完成后重启 Gateway 才会生效。"
          done={step1Done} active={!step1Done}
        >
          <div style={{ marginTop: 8, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <button
              className={`btn btn-sm ${step1Done ? 'btn-success' : 'btn-primary'}`}
              onClick={installPlugin}
              disabled={pluginState === 'loading' || step1Done}
            >
              {pluginState === 'loading' ? <><span className="spin">↻</span> 安装中...</>
                : step1Done ? '✓ 已安装'
                : '一键安装插件'}
            </button>
            {pluginState === 'error' && (
          ...
          {!step1Done && (
            <div className="form-hint" style={{ marginTop: 6, color: 'var(--muted)' }}>
        ...
        <StepCard
          ...
          title={<>在钉钉开放平台创建应用 <ManualTag>手动</ManualTag></> as any}
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, color: 'var(--muted2)' }}>
            ...
            <div>
              ③ 机器人配置页 → <strong style={{ color: 'var(--yellow)' }}>消息接收模式选「Stream 模式」</strong>（不是 HTTP 回调）
            </div>
        ...
        <StepCard
          ...
          title={<>获取应用凭据 <ManualTag>手动</ManualTag></> as any}
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, color: 'var(--muted2)', marginTop: 8 }}>
            <div>
              · <strong>Client ID（AppKey）</strong>：凭据与基础信息 → Client ID
            </div>
            <div>
              · <strong>Client Secret（AppSecret）</strong>：凭据与基础信息 → Client Secret
            </div>
            <div>
              · <strong>CorpId</strong>：凭据与基础信息 → 企业 ID（CorpId）
            </div>
            <div>
              · <strong>AgentId</strong>：凭据与基础信息 → AgentId（数字）
            </div>
          </div>
        </StepCard>
        ...
        {/* Step 4: 填写凭据并写入配置 */}
        <StepCard
          num={4} title="填写凭据并写入 OpenClaw 配置"
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
            ...
            <div>
              <button
                ...
                disabled={channelState === 'loading' || !formFilled}
              >
        ...
        {/* Step 5: 启动 / 重启 Gateway */}
        <StepCard
          num={5} active={step4Done}
          title="重启 OpenClaw Gateway"
          desc="配置写入后，需要重启 Gateway 使插件生效。前往「服务面板」点击重启，等状态变为「运行中」。"
        >
        ...
        {/* Step 6: 测试 */}
        <StepCard
          num={6} active={step4Done}
          title={<>在钉钉中测试机器人 <ManualTag>手动</ManualTag></> as any}
          ...
          <div className="step done">
            ...
            <div className="step-body">
              ...
              <div className="step-desc">
                钉钉机器人已就绪。重启 Gateway 后即可在钉钉中与机器人对话。
              </div>
              <span className="badge badge-green" style={{ marginTop: 6 }}>
                <span className="dot dot-pulse"></span>钉钉 Channel 已激活
              </span>
            </div>
          </div>
        )}
